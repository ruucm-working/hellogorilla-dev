		...
		*/
		public function check_payment_response() {
			...
			if ( !empty($http_param['imp_uid']) ) {
				...
				$called_from_iamport ? exit( json_encode(array("version"=>"IamportForWoocommerce 2.0.55", "log"=>$loggers)) ) : wp_redirect( $default_redirect_url );
			} else {
				//just test(아임포트가 지원하는대로 호출되지 않음)
				exit( json_encode(array("version"=>"IamportForWoocommerce 2.0.55")) );
			}
		}
		...
		public function enqueue_iamport_script() {
			...
			wp_register_script( 'iamport_script_for_woocommerce', plugins_url( '/assets/js/iamport.woocommerce.js',plugin_basename(__FILE__) ), array('jquery'), '20180626');
