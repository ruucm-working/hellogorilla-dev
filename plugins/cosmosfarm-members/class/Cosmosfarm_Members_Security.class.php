...
final class Cosmosfarm_Members_Security {
	...
	public function get_user_ip(){
	    ...
	    if(isset($_SERVER['HTTP_CLIENT_IP']) && $_SERVER['HTTP_CLIENT_IP'] && filter_var($_SERVER['HTTP_CLIENT_IP'], FILTER_VALIDATE_IP)){
	        $ip = $_SERVER['HTTP_CLIENT_IP'];
	    }
	    elseif(isset($_SERVER['HTTP_X_FORWARDED_FOR']) && $_SERVER['HTTP_X_FORWARDED_FOR'] && filter_var($_SERVER['HTTP_X_FORWARDED_FOR'], FILTER_VALIDATE_IP)){
	...
	public function get_user_country(){
		...
		if(is_array($response) && $response['response']['code'] == '200'){
			...
			$country->country_name = $data->geoplugin_countryName;
			$country->country_code = $data->geoplugin_countryCode;
			return $country;
		}
		else{
			$country = new stdClass();
	...
	public function save_history_user_role_change($user_id, $role, $old_roles){
		$option = get_cosmosfarm_members_option();
		if($option->save_activity_history){
			$this->add_activity_history(get_current_user_id(), $user_id, "사용자 역할 변경 ({$role})");
		}
	}
